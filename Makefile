...
build:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-extldflags=-static -w -s -X github.com/NoF0rte/chaakoo/cmd.version=$(version)' -o "chaakoo-$(version)-linux-amd64" cmd/chaakoo/main.go
